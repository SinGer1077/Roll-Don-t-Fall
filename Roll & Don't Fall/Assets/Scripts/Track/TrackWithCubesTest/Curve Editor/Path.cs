...
public class Path
{
    ...
    public bool AutoSetControlPoint
    {
        get
        {
    ...
    public Vector3[] CalculateEvenlySpacedPoints(float spacing, float resolution = 1)
    {
        ...
        for (int segmentIndex = 0; segmentIndex < NumSegments; segmentIndex++)
        {
            ...
            float controlNewLength = Vector3.Distance(p[0], p[1]) + Vector3.Distance(p[1], p[2]) + Vector3.Distance(p[2], p[3]);
            float estimatedCurveLength = Vector3.Distance(p[0], p[3]) + controlNewLength / 2f;
